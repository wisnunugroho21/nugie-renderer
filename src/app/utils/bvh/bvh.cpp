...
namespace NugieApp {
  ...
  Aabb PrimitiveBoundBox::boundingBox() {
    return Aabb { 
      glm::vec3(glm::min(glm::min(this->vertices[this->primitive->indices.x].position, this->vertices[this->primitive->indices.y].position), this->vertices[this->primitive->indices.z].position)) - eps,
      glm::vec3(glm::max(glm::max(this->vertices[this->primitive->indices.x].position, this->vertices[this->primitive->indices.y].position), this->vertices[this->primitive->indices.z].position)) + eps
    };
  }

  Aabb TriangleLightBoundBox::boundingBox() {
    return Aabb { 
      glm::vec3(glm::min(glm::min(this->vertices[this->light->indices.x].position, this->vertices[this->light->indices.y].position), this->vertices[this->light->indices.z].position)) - eps,
      glm::vec3(glm::max(glm::max(this->vertices[this->light->indices.x].position, this->vertices[this->light->indices.y].position), this->vertices[this->light->indices.z].position)) + eps
    };
  }
