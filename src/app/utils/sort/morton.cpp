...
namespace NugieApp {
  ...
  std::vector<ScreenRayCoord> sortPixelByMorton(uint32_t width, uint32_t height) {
    auto pixels = std::vector<ScreenRayCoord>();

    for (uint32_t i = 0; i < width; i++) {
      for (uint32_t j = 0; j < height; j++) {
        // uint32_t morton = encodeMorton(i, j);
        pixels.emplace_back(ScreenRayCoord{ glm::uvec2(i, j) });
      }
    }

    // std::sort(pixels.begin(), pixels.end(), mortonComparator);

    /* auto results = std::vector<ScreenRayCoord>();
    for (auto &&pixel: pixels) {
      results.emplace_back(ScreenRayCoord{ glm::uvec2(pixel.coord.x, pixel.coord.y) });
    } */
