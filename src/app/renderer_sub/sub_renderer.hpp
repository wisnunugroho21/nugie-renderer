...
namespace NugieApp {
    enum AttachmentType {
        KEEPED = 0, INPUT_OUTPUT = 1, OUTPUT_SHADER = 2, OUTPUT_TEXTURE = 3, OUTPUT_STORED = 4
    };
    enum AttachmentRole {
        COLOR = 0, DEPTH = 1, RESOLVED = 2
    };
    ...
    public:
        class Builder {
        public:
            Builder(NugieVulkan::Device *device, uint32_t width, uint32_t height, uint32_t imageCount,
                    uint32_t layerNum = 1u);

            Builder &addAttachment(AttachmentType attachmentType, VkFormat format, VkImageLayout layout,
                                   VkSampleCountFlagBits sample);

            Builder &addAttachment(const std::vector<NugieVulkan::Image *> &attachments, AttachmentType attachmentType,
                                   VkFormat format,
                                   VkImageLayout layout, VkSampleCountFlagBits sample);

            Builder &setDepthAttachment(AttachmentType attachmentType, VkFormat format, VkImageLayout layout,
                                        VkSampleCountFlagBits sample);

            Builder &
            setDepthAttachment(const std::vector<NugieVulkan::Image *> &attachments, AttachmentType attachmentType,
                               VkFormat format,
                               VkImageLayout layout, VkSampleCountFlagBits sample);
            ...
            Builder &
            addResolvedAttachment(const std::vector<NugieVulkan::Image *> &attachments, AttachmentType attachmentType,
                                  VkFormat format,
        ...
        class Overwriter {
        public:
            Overwriter(NugieVulkan::Device *device, uint32_t width, uint32_t height, uint32_t imageCount,
                       uint32_t layerNum = 1u);
        ...
        SubRenderer(NugieVulkan::Device *device, uint32_t width, uint32_t height, uint32_t layerNum,
        ...
        getAttachmentInfos(uint32_t subpassIndex) const { return this->attachmentInfos[subpassIndex]; }
        ...
        void recreateResources(uint32_t width, uint32_t height, uint32_t imageCount,
    ...
    private:
        ...
        void createRenderPass(const std::vector<std::vector<NugieVulkan::Image *>> &attachments,
                              const std::vector<VkAttachmentDescription> &attachmentDescs,
                              const std::vector<std::vector<VkAttachmentReference>> &outputAttachmentRefs,
                              const std::vector<VkAttachmentReference> &depthAttachmentRefs,
                              const std::vector<std::vector<VkAttachmentReference>> &inputAttachmentRefs,
