...
namespace NugieApp {
	TerrainPassRenderSystem::TerrainPassRenderSystem(NugieVulkan::Device *device,
													 ...
													 bool isRasterLineMode)
			: GraphicRenderSystem(device, renderPass, vertFilePath, fragFilePath, descriptorSetLayouts, pushConstantRanges), 
			  tescFilePath{tescFilePath}, teseFilePath{teseFilePath}, isRasterLineMode{isRasterLineMode} {
	}
	...
	void TerrainPassRenderSystem::createPipeline() {
		...
		auto *inputAssemblyInfo = new VkPipelineInputAssemblyStateCreateInfo();
		...
		auto *tessellationInfo = new VkPipelineTessellationStateCreateInfo();
