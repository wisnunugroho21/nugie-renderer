...
namespace NugieApp {
	...
	void App::loadObjects() {
		...
		this->lightTransformationModel = new LightTransformationModel(this->device);
		this->lightTransformationModel->update(commandBuffer, lightTransformations);
	...
	void App::recreateSubRendererAndSubsystem() {
		...
		this->modelDeferredDescSet = new ModelDeferredDescSet(this->device, this->renderer->getDescriptorPool(), deferredUniformInfo, deferredModelInfo, 
			this->shadowSubPartRenderer->getDepthPointInfoResources());
