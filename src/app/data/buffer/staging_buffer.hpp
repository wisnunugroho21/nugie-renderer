...
namespace NugieApp {
	...
	class StagingBuffer {
		public:
			StagingBuffer(NugieVulkan::Device* device, VkBufferUsageFlags usageFlags);
			~StagingBuffer();
	...
	template <typename T>
	StagingBuffer<T>::StagingBuffer(NugieVulkan::Device* device, VkBufferUsageFlags usageFlags) : device{device} {
	...
	template <typename T>
	std::vector<VkDescriptorBufferInfo> StagingBuffer<T>::getInfo(VkDeviceSize size, VkDeviceSize offset) const {
	...
	template <typename T>
	void StagingBuffer<T>::createBuffer(VkBufferUsageFlags usageFlags) {
		this->buffers.clear();
	...
	template <typename T>
	void StagingBuffer<T>::writeGlobalData(uint32_t frameIndex, T ubo, VkDeviceSize size, VkDeviceSize offset) {
