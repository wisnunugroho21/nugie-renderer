...
namespace NugieApp {
  ...
  StackedArrayManyBuffer::Builder& StackedArrayManyBuffer::Builder::addArrayItem(const std::string& id, VkDeviceSize instanceSize, uint32_t count) {
    this->arrayItemInfos.emplace_back(ArrayItemInfo{
      id,
      instanceSize,
		  count
  ...
  std::vector<VkDescriptorBufferInfo> StackedArrayManyBuffer::getInfo(std::string arrayId) {
  ...
  void StackedArrayManyBuffer::transitionBuffer(NugieVulkan::CommandBuffer* commandBuffer, uint32_t bufferIndex, std::string arrayId, 
		VkPipelineStageFlags srcStage, VkPipelineStageFlags dstStage, VkAccessFlags srcAccess, VkAccessFlags dstAccess, 
		uint32_t srcQueueFamilyIndex, uint32_t dstQueueFamilyIndex)
  {
