...
void main() {
  ...
  vec4 lightCoord = shadowTransformations[0].projection * shadowTransformations[0].view * inPosition;
  lightCoord.xyz = lightCoord.xyz / lightCoord.w;
  lightCoord.xy = lightCoord.xy * 0.5 + 0.5;

  float shadowFactor = texture(shadowMapTexture[0], lightCoord.xy).x < lightCoord.z && lightCoord.w > 0.0f 
    ? 0.1f 
    : 1.0f;
